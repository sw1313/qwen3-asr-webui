...
def vllm_worker_queue(
    ...
    *,
    ...
    """
    Persistent vLLM worker: initialize the engine ONCE, then process batches sent by parent.

    Parent -> cmd_q messages:
      - ("run_files", (batch_id_int, [file_path_str, ...]))
      - ("shutdown", None)

    Worker -> out_q messages:
      - ("init", None)
      - ("batch_start", (batch_id, total))
      - ("file_start", (batch_id, idx1, total, file_path, filename))
      - ("progress", (batch_id, file_path, pct_int_0_100))
      - ("file_done", (batch_id, idx1, total, file_path, log_line, out_path_str_or_None))
      - ("file_error", (batch_id, idx1, total, file_path, message))
      - ("batch_done", (batch_id,))
      - ("cancelled", None)
    """
    # Put this worker in its own process group/session so parent can kill the whole tree
    ...
    try:
        ...
        while True:
            if cancel_ev is not None and cancel_ev.is_set():
                ...
                break
            try:
                cmd_type, payload = cmd_q.get(timeout=0.2)
            except Exception:
                continue
            if cmd_type == "shutdown":
                break
            if cmd_type != "run_files":
                continue
            try:
                batch_id, files = payload
                batch_id_n = int(batch_id)
                files_l = list(files or [])
            except Exception:
                continue

            total = len(files_l)
            out_q.put(("batch_start", (batch_id_n, total)))
            for i0, f in enumerate(files_l):
                if cancel_ev is not None and cancel_ev.is_set():
                    out_q.put(("cancelled", None))
                    return
                ap = Path(str(f))
                out_q.put(("file_start", (batch_id_n, i0 + 1, total, str(ap), ap.name)))
                cur_pct = 0
                ...
                def _progress_cb(pct: int, _stage: str = "") -> None:
                    ...
                    if pct_i <= cur_pct:
                        return
                    cur_pct = max(0, min(100, pct_i))
                    out_q.put(("progress", (batch_id_n, str(ap), cur_pct)))
                ...
                try:
                    ...
                    if ext and not (cancel_ev is not None and cancel_ev.is_set()):
                        ...
                        out_path_s = str(outp)
                    _progress_cb(100, "done")
                    out_q.put(("file_done", (batch_id_n, i0 + 1, total, str(ap), log_line, out_path_s)))
                except Exception as e:
                    out_q.put(("file_error", (batch_id_n, i0 + 1, total, str(ap), str(e))))

            out_q.put(("batch_done", (batch_id_n,)))
    except Exception as e:
        try:
            out_q.put(("file_error", (-1, 0, 0, "", str(e))))
        except Exception:
            pass
    finally:
        # Best-effort VRAM cleanup.

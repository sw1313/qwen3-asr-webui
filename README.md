...
- **智能后处理（默认关闭）**  
  完全参考 `Qwen3-ASR-Toolkit` 的 `post_text_process`：自动检测并去除常见重复伪影（字符级重复、模式级重复），用于获得更干净的转录文本。

- **vLLM 常驻引擎（更快且稳定）**  
  在 vLLM 后端下，模型/引擎可常驻一个子进程：按文件夹逐批次喂给引擎执行，避免反复初始化带来的时间与显存波动。

- **配置自动保存（NAS 友好）**  
  所有控件修改会自动写入 `data/webui_config.json`（滑块/下拉/勾选/文本框均可实时保存）；刷新页面会从磁盘重新加载最新配置，重启容器后仍保留配置。
...
- **`WEBUI_CONFIG`** / **`WEBUI_CONFIG_PATH`**：WebUI 配置文件路径  
  ...
  - 行为：WebUI 所有控件修改会自动保存（包括滑块拖动与文本框输入），并在页面加载/刷新时从磁盘回填，避免旧缓存覆盖新值。
...
- 位置：`模型参数 -> 高级参数` 中的“智能后处理（Qwen3-ASR-Toolkit 同款）”
- 逻辑：完全参考 `Qwen3-ASR-Toolkit` 的 `post_text_process`（先字符重复压缩，再模式重复压缩）
- 参数：`threshold` 默认 `20`（越小越“激进”）
- 建议：仅在你观察到明显重复/回环文本时开启；正常场景建议保持关闭以避免过度清理。

### 幻觉检测重试（可选，默认关闭）

- 位置：独立主折叠面板 `幻觉检测重试`（不在 `其它` 中，非 JSON）
- 总开关：`幻觉检测重试`
- 重试层级：
  - **段级重试**：关键词命中时对单个 VAD 段重试（可配次数）
  - **整段重试**：命中整段规则时对整音视频重跑（可配次数）
- 新增规则：
  - **无输出结果重试规则**（不含“无音轨/静音”）
  - **整段文本由重复子串构成重试规则**
  - **文本中有连续重复子串重试规则**
  - **字幕尾时间戳与音频时长差值 > N 秒重试规则**
  - **超长段数量 >= N 个重试规则**
  - **合并后出现超过 N 秒的段则重试规则**
- 两条“重复子串”规则均支持：
  - **重复子串最少长度**
  - **该段重复子串最少出现次数**
  - **命中规则最少 N 次数才重试**（按“一个字幕时间戳段=一次命中”统计）
- 整段重试可选增强：
  - **整个音视频重试使用另一组 VAD 参数**（threshold / min&max speech / silence / pad / window / merge gap / min segment）
- 日志增强：
  - 重试日志会输出命中原因、命中字幕段、文本片段、所有命中详情与重试耗时。
